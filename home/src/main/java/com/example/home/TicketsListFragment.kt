...
class TicketsListFragment : Fragment(R.layout.fragment_tickets_list) {
    ...
    private fun initFilter() {
        binding.fTicketToolbar.setOnMenuItemClickListener {
            when (it.itemId) {
                coreR.id.m_home_app_bar_priority -> {
                    createFilterDialog(viewModel.priorities, viewModel.checkedPriorities, getString(coreR.string.by_priority), ITicketEnum::getName)
                    true
                }
                coreR.id.m_home_app_bar_status -> {
                    createFilterDialog(viewModel.statuses, viewModel.checkedStatuses, getString(coreR.string.by_status), ITicketEnum::getName)
                    true
                }
                coreR.id.m_home_app_bar_service -> {
                    createFilterDialog(viewModel.services, viewModel.checkedServices, getString(coreR.string.by_service), ITicketEnum::getName)
                    true
                }
                coreR.id.m_home_app_bar_kind -> {
                    createFilterDialog(viewModel.kinds, viewModel.checkedKinds, getString(coreR.string.by_kind), ITicketEnum::getName)
                    true
                }
                coreR.id.m_home_app_bar_creation_date -> {
        ...
        val drawable = ContextCompat.getDrawable(
            requireContext(),
            coreR.drawable.ic_baseline_filter_list_24_white
        )
