...
class GetTicketsHistoryUseCase @Inject constructor(
    ...
    suspend fun execute(url: String?, currentUserId: Long?): Pair<INetworkState?, List<TicketEntity>?> {
        ...
        val result = ticketsHistoryRepository.getTicketsHistory("$url${Endpoints.Tickets.GET_HISTORY}/$currentUserId")

        return when (result.first) {
            200 -> Pair(null, result.second)
            else -> {
                Logger.m("Unresolved error while getting ticket history. Status code: ${result.first}")
